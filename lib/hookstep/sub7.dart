...
final addressProvider =
    StateProvider<Addr>((ref) => Addr(address: '', postalCode: '')); // 初期値は空文字

class Addr {
  String address;
  String postalCode;

  Addr({key, required this.address, required this.postalCode});
}

// APIから住所を取得する非同期関数
Future<Addr> fetchAddress(String postalCode) async {
  ...
  if (response.statusCode == 200) {
    ...
    return Addr(address: address, postalCode: postalCode); // APIから取得した住所を返す
  } else {
...
class HomeScreen extends HookConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final addr = ref.watch(addressProvider); // プロバイダから住所を監視
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: [
            Text(
              '住所: ${addr.address} 郵便番号: ${addr.postalCode}', // 住所を表示
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                try {
                  // APIから住所を非同期で取得
                  final addr = await fetchAddress(postalCode);
                  // 取得した住所をプロバイダに保存
                  ref.read(addressProvider.notifier).state = addr;
                  // 元の画面に戻る
                  if (context.mounted) {
